...
# make_loan_repayment_entry was removed in hrms v16 — import conditionally
try:
    from hrms.payroll.doctype.salary_slip.salary_slip import make_loan_repayment_entry as _make_loan_repayment_entry
    _HAS_MAKE_LOAN_REPAYMENT_ENTRY = True
except ImportError:
    _HAS_MAKE_LOAN_REPAYMENT_ENTRY = False
...
class CustomSalarySlip(SalarySlip):
    def on_submit(self):
        ...
        if _HAS_MAKE_LOAN_REPAYMENT_ENTRY:
            _make_loan_repayment_entry(self)
...
# show_payroll_submission_status signature/location may vary across hrms versions
try:
    ...
    def show_payroll_submission_status(submitted, unsubmitted, payroll_entry):
        if unsubmitted:
            frappe.msgprint(
                _("Could not submit {0} salary slips. Check Error Log for details.").format(len(unsubmitted))
            )
        if submitted:
            frappe.msgprint(_("{0} salary slips submitted successfully.").format(len(submitted)))

# link_bank_entry_in_salary_withholdings was introduced in hrms v15 — import conditionally
try:
    from hrms.payroll.doctype.salary_withholding.salary_withholding import link_bank_entry_in_salary_withholdings
    _HAS_LINK_BANK_ENTRY = True
except ImportError:
    _HAS_LINK_BANK_ENTRY = False
...
class CustomPayrollEntry(PayrollEntry):
    ...
    def make_bank_entry(self, for_withheld_salaries: bool = False) -> Optional[Document]:
        ...
        if salary_slip_total != 0:
            ...
            if for_withheld_salaries and _HAS_LINK_BANK_ENTRY:
